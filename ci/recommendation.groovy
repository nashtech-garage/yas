/**
 * Recommendation Service CI - loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl recommendation -am)
 * Phases     : Phase 1 - Test (unit tests w/ 80% coverage gate, checkstyle, OWASP dependency-check)
 *              Phase 2 - Build artifacts + Docker image
 */

def call() {
    def serviceName    = 'recommendation'
    ...
    try {
        ...
        stage("${serviceName}: Phase 1 - Unit Tests") {
            try {
                ...
                junit testResults: 'recommendation/**/surefire-reports/TEST*.xml',
        ...
        stage("${serviceName}: Phase 1 - Code Quality") {
            ...
            echo 'Checkstyle report generated at: recommendation-checkstyle-result.xml'
        }
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl recommendation -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
                ...
                publishHTML target: [
                    ...
                    keepAll              : true,
                    reportDir            : 'recommendation/target',
        ...
        echo 'Recommendation service build completed successfully'
    } catch (Exception e) {
