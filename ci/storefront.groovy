/**
 * Storefront Service CI — loaded and executed by the master Jenkinsfile.
 *
...
def call() {
    ...
    try {
                    ...
                    withEnv(['NODE_OPTIONS=--max-old-space-size=4096']) {
                        sh '''
                            if node -e "const p=require('./package.json'); const hasJest=Boolean((p.devDependencies&&p.devDependencies.jest)||(p.dependencies&&p.dependencies.jest)||(p.scripts&&p.scripts.test)); process.exit(hasJest?0:1)"; then
                                ...
                                echo 'No Jest configuration found in storefront/package.json. Skipping unit tests.'
                            fi
                        '''
                    }
                }
            } finally {
                junit testResults: 'storefront/junit.xml', allowEmptyResults: true
                publishHTML([
                    allowMissing         : true,
        ...
        echo "✓ Storefront service build completed successfully"
    } catch (Exception e) {
        echo "✗ Storefront service build failed"
        throw e
    }
}
