/**
 * Rating Service CI - loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl rating -am)
...
def call() {
    ...
    try {
        ...
        stage("${serviceName}: Phase 1 - Code Quality") {
            ...
            echo 'Checkstyle report generated at: rating-checkstyle-result.xml'
        }
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl rating -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
        ...
        echo 'Rating service build completed successfully'
    } catch (Exception e) {
        echo 'Rating service build failed'
        throw e
    }
}
