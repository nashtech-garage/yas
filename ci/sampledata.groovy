/**
 * Sampledata Service CI — loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl sampledata -am)
 * Phases     : Phase 1 — Test (unit tests w/ 80% coverage gate, checkstyle, OWASP dependency-check)
 *              Phase 2 — Build artifacts + Docker image
 */

def call() {
    def serviceName    = 'sampledata'
    ...
    try {
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl sampledata -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
        ...
        echo "Sampledata service build completed successfully"
    } catch (Exception e) {
        echo "Sampledata service build failed"
        throw e
    }
}
