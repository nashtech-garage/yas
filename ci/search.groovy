/**
 * Search Service CI — loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl search -am)
 * Phases     : Phase 1 — Test (unit tests w/ 80% coverage gate, checkstyle, OWASP dependency-check)
 *              Phase 2 — Build artifacts + Docker image
 */

def call() {
    def serviceName    = 'search'
    ...
    try {
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl search -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
        ...
        echo "Search service build completed successfully"
    } catch (Exception e) {
        echo "Search service build failed"
        throw e
    }
}
