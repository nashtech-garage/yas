/**
 * Payment Service CI - loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl payment -am)
    ...
    try {
        ...
        stage("${serviceName}: Phase 1 - Unit Tests") {
            try {
                ...
                junit testResults: 'payment/**/surefire-reports/TEST*.xml',
        ...
        stage("${serviceName}: Phase 1 - Code Quality") {
            ...
            echo 'Checkstyle report generated at: payment-checkstyle-result.xml'
        }
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl payment -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
