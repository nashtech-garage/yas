/**
 * Payment-PayPal Service CI - loaded and executed by the master Jenkinsfile.
 *
 * Tech stack : Java / Maven (multi-module, uses -pl payment-paypal -am)
 * Phases     : Phase 1 - Test (unit tests w/ 80% coverage gate, checkstyle, OWASP dependency-check)
 *              Phase 2 - Build artifacts + Docker image
 */

def call() {
    def serviceName    = 'payment-paypal'
    ...
    try {
        ...
        stage("${serviceName}: Phase 1 - Unit Tests") {
            try {
                ...
                junit testResults: 'payment-paypal/**/surefire-reports/TEST*.xml',
        ...
        stage("${serviceName}: Phase 1 - Code Quality") {
            ...
            echo 'Checkstyle report generated at: payment-paypal-checkstyle-result.xml'
        }
            ...
            try {
                try {
                    withCredentials([string(credentialsId: 'nvd-api-key', variable: 'NVD_API_KEY')]) {
                        sh '''
                            mvn -pl payment-paypal -am org.owasp:dependency-check-maven:check \
                              -DfailBuildOnCVSS=7 \
                ...
                publishHTML target: [
                    ...
                    keepAll              : true,
                    reportDir            : 'payment-paypal/target',
        ...
        echo 'Payment-PayPal service build completed successfully'
    } catch (Exception e) {
        echo 'Payment-PayPal service build failed'
        throw e
    }
}
