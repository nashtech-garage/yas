...
public class ControllerAdvisor {
    ...
    public ResponseEntity<ErrorVm> handleUnsupportedMediaTypeException(UnsupportedMediaTypeException ex, WebRequest request) {
        ErrorVm errorVm = new ErrorVm(HttpStatus.BAD_REQUEST.toString(), "Unsupported media type", "File uploaded media type is not supported");
    ...
    protected ResponseEntity<ErrorVm> handleMethodArgumentNotValid(MethodArgumentNotValidException ex) {
        ...
        ErrorVm errorVm = new ErrorVm(HttpStatus.BAD_REQUEST.toString(), "Bad Request", "Request information is not valid", errors);
        return ResponseEntity.badRequest().body(errorVm);
    }
