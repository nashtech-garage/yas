...
| **TC-CD-04** | BASIC-08 BASIC-09 | NodePort URL works | Services deployed in K8s | 1. Run `kubectl get svc -n dev` to identify NodePort.<br>2. Navigate to `http://<domain_name>:<nodeport>`. | The YAS Storefront or Backoffice homepage loads correctly using the NodePort URL. | `UNVERIFIED_RUNTIME` |
| **TC-CD-05** | BASIC-10 | hosts file mapping works | Local OS hosts file configured | 1. Add mapping: `<Worker-IP> <domain_name>`.<br>2. Ping the `<domain_name>`. | The ping resolves to the exact IP of the Kubernetes Worker node and verifies DNS resolution. | `UNVERIFIED_RUNTIME` |
| **TC-CD-06** | BASIC-11 | CD Cleanup Job | Deployment exists from `developer_build` | 1. Trigger the cleanup Jenkins job.<br>2. Verify pod/svc deletion in namespace. | All Kubernetes deployments created by `developer_build` are deleted. | `UNVERIFIED_RUNTIME` |
