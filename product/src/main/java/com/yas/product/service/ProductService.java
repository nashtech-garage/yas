...
public class ProductService {
    ...
    public ListObjectAndPageVm<List<ProductRatingVm>> getListRatingByProductId(Long id, int pageNo, int pageSize) {
        if (productRepository.findById(id).isEmpty()) {
            throw new NotFoundException(Constants.ERROR_CODE.PRODUCT_NOT_FOUND, id);
        }

        Pageable pageable = PageRequest.of(pageNo, pageSize);
        Page<ProductRating> productRatingPage = productRatingRepository.findByProduct_Id(id, pageable);

        List<ProductRatingVm> productRatingVmList = new ArrayList<>();;

        for (ProductRating productRating : productRatingPage.getContent()) {
            productRatingVmList.add(new ProductRatingVm(
                    productRating.getId(),
        ...
        return new ListObjectAndPageVm<>(productRatingVmList, productRatingPage.getTotalPages());
    }
}
