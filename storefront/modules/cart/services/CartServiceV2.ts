...
export async function addCartItem(cartItemPostVm: CartItemPostVm): Promise<CartItemGetVm> {
  const response = await apiClientService.post(CART_BASE_URL, JSON.stringify(cartItemPostVm));
  if (!response.ok) {
...
export async function getDetailedCartItems(): Promise<CartItemGetDetailsVm[]> {
  ...
  const products = await getProductByIds(cartItemProductIds);
...
async function getProductByIds(ids: number[]): Promise<ProductThumbnail[]> {
...
export async function bulkDeleteCartItems(productIds: number[]): Promise<CartItemGetVm[]> {
  const response = await apiClientService.post(
    `${CART_BASE_URL}/remove`,
...
export async function updateCartItem(productId: number, quantity: number): Promise<CartItemGetVm> {
